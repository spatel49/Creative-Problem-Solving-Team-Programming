 ...
 * Date        : 3/8/21
 * Description : Scramble Squares Puzzle Solver.
...
Tile Board[3][3]=               //Main Board with 9 tiles
{
    {Tile(),Tile(),Tile()},
    {Tile(),Tile(),Tile()},
    {Tile(),Tile(),Tile()},
...
bool is_prev_soln(string S,int R[9]){       // function taht checks whether a current board already exists in solutions
    ...
    {
        if(State[i]==S1) return true;       //If a solution already exist in the State vector 
    }
...
{
    int maincount = 0; //counter for the length of the input file
    if(argc<2){
    ...
    vector<Tile> Puzzle;            //vector to store a tile
    ...
    for(int i=0;i<9;i++)
    {
        ...
        if (input_line.length() > 0){ //check the content's length of input file
            maincount++;              
        }
    ...
    for(int i=0;i<maincount;i++)        // Printing the input 
    {
